...
## The Challenge
The source dataset contains a denormalized column (`line_items`) where multiple products per order are stored as a **JSON array**. This structure is unsuitable for standard relational analysis and requires flattening to achieve **First Normal Form (1NF)**.
...
* **Logic Design:** Builds reusable **Common Table Expressions (CTEs)** to aggregate data at the order level.
* **Calculation:** Computes total sales per order ($Price \times Quantity$) and a grand total for efficiency.
